<form action="{{ route('jenis-kegiatans.update', $jenisKegiatan->JenisKegiatanID) }}" method="POST" class="modal-form" id="jenisKegiatanEditForm">
    @csrf
    @method('PUT')
    ...
    <div class="form-group">
        ...
        <select name="NA" id="NA" class="form-control">
            <option value="Y" {{ $jenisKegiatan->NA == 'Y' ? 'selected' : '' }}>Non Aktif</option>
            <option value="N" {{ $jenisKegiatan->NA == 'N' ? 'selected' : '' }}>Aktif</option>
        </select>
    </div>
...
<script>
document.getElementById('jenisKegiatanEditForm').addEventListener('submit', function(event) {
